...
class EncoderReaderBridge(Node):
    def __init__(self, factory, enabled=True):
        ...
        self.rel_enc_ticks_to_rad = 2 * math.pi / 2000
        self.motor_enc_ticks_to_rad = 2 * math.pi / (131.25 * 64)
        self.abs_gear_ratio = 48.0/32.0
        ...
        while self.factory.ok():
            ...
            if packet.name is None:
                ...
                if time.time() - self.prev_broadcast_time > 0.25:
                    ...
                    packet.data[0] *= math.pi / 180 * self.abs_gear_ratio
                    packet.data[1] *= math.pi / 180 * self.abs_gear_ratio

                    packet.data[0] -= self.initial_abs_enc1
                    packet.data[1] -= self.initial_abs_enc2

                    packet.data[4] *= self.rel_enc_ticks_to_rad
                    packet.data[5] *= self.rel_enc_ticks_to_rad

                    packet.data[6] *= self.motor_enc_ticks_to_rad
